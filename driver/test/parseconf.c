...
static int findSectionEnd(FILE *f)
{
	FILE tmp = *f;
	char str[512];
	char *string = fgets(str, sizeof(str), f);
	*f = tmp;
	return strchr(string, ']') ? 1 : 0;
}

static int atSectionEnd(FILE *f)
{
	FILE tmp = *f;
	char c = getc(f);
	*f = tmp;
...
static int section( FILE *InFile, int (*sfunc)(const char *) )
{
        int   c;
        int   i;
        int   end;
        ...
        if (!findSectionEnd(InFile)) {
                fprintf(stderr, "%s:%s() - No terminating ']' character in section.\n", FILENAME, __func__ );
                return -1;
        }
        ...
        while( (EOF != c) && (c > 0) ) {
                ...
                if( i > (bSize - 2) ) {
                        char *tb = (char *)realloc( bufr, bSize +BUFR_INC );
                        if(!tb) {
                                fprintf(stderr, "%s:%s() - Memory re-allocation failure.", FILENAME, __func__ );
                                return -1;
                        }
                        bufr = tb;
                        bSize += BUFR_INC;
                ...
                switch( c ) {
                        case '\n': /* Got newline before closing ']'.    */
                                ...
                                if( i < 0 ) {
                                        bufr[end] = '\0';
                                        fprintf(stderr, "%s:%s() - Badly formed line in configuration file: %s\n", FILENAME, __func__, bufr );
                                        return -1;
                                }
				...
				if (c == ']') {
                        ...
                        if( 0 == end ) {
                                ...
                                fprintf(stderr, "%s:%s() - Empty section name in configuration file.\n", FILENAME, __func__ );
                                return -1;
                        }
        ...
        fprintf(stderr, "%s:%s() - Unexpected EOF in the configuration file: %s\n", FILENAME, __func__, bufr );
        return -1;
}
...
static int parameter( FILE *InFile, int (*pfunc)(const char *, const char *), int c )
{
        ...
        while( 0 == vstart ) {
                ...
                switch(c) {
                        case '=': /* Equal sign marks end of param name. */
                                if( 0 == end ) {
                                        /* Don't allow an empty name.      */
                                        fprintf(stderr, "%s:%s() - Invalid parameter name in config. file.\n", FILENAME, __func__ );
                                        return -1;
                                }
                        ...
                        case '\n': /* Find continuation char, else error. */
                                ...
                                if( i < 0 ) {
                                        bufr[end] = '\0';
                                        fprintf(stderr, "%s:%s() - Ignoring badly formed line in configuration file: %s\n", FILENAME, __func__, bufr );
                                        return 0;
                                }
                                end = ( (i > 0) && (' ' == bufr[i - 1]) ) ? (i - 1) : (i);
                        ...
                        case EOF:
                                bufr[i] = '\0';
                                fprintf(stderr, "%s:%s() - Unexpected end-of-file at: %s\n", FILENAME, __func__, bufr );
                                return 0;
...
static FILE *openConfFile( const char *FileName )
{
	...
	fp = fopen(FileName, "r");
	if (fp == NULL) {
		fprintf(stderr, "%s:%s() - Unable to open configuration file \"%s\"\n",
				FILENAME, __func__, FileName );
		exit(-1);
	}
	return fp;
}
 ...
 * ------------------------------------------------------------------------ **
 */
static int pm_process( const char *FileName,
		int (*sfunc)(const char *),
		int (*pfunc)(const char *, const char *) )
{
	FILE *fp;
	...
	fclose(fp);
	if( !result ) {
		fprintf(stderr, "%s:%s() - Failed.  Error returned from params.c:parse().\n", FILENAME, __func__);
		return -1;
	}
...
int dummyfunc1(const char *a) {
	printf("section: %s\n", a);
	return 0;
}
int dummyfunc2(const char *a, const char *b) {
...
{
	pm_process(argv[1], &dummyfunc1, &dummyfunc2);
	return 0;
}
